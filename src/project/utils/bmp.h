...
struct BMP {
        ...
        if (inp) {
            ...
            if (bmp_info_header.bit_count == 32) {
                ...
                if (bmp_info_header.size >= (sizeof(BMPInfoHeader) + sizeof(BMPColorHeader))) {
                    ...
                    winColor = check_color_header(bmp_color_header);
                } else {
            ...
            if (bmp_info_header.width % 4 == 0) {
                ...
                for (int y = 0; y < bmp_info_header.height; ++y) {
                    ...
                    inp.read((char *) padding_row.data(), (uint32_t) padding_row.size());
                }
                file_header.file_size += (uint32_t) (data.size() + bmp_info_header.height * padding_row.size());
            }
            ...
            if (winColor) {
                ...
                for (int y = h - 1; y >= 0; --y) {
                    int32_t offset = y * w * 4;
                    for (int x = 0; x < w; ++x) {
                        uint8_t b = d[offset + x * 4 + 0];
                        uint8_t g = d[offset + x * 4 + 1];
                        uint8_t r = d[offset + x * 4 + 2];
                        uint8_t a = d[offset + x * 4 + 3];
                        //printf("[%u,%u] = a:%u, r:%u, g:%u, b:%u\n", x, realY, a, r, g, b);
                        fd.push_back(r);
                        fd.push_back(g);
                        fd.push_back(b);
                        fd.push_back(a);
    ...
    BMP(int32_t width, int32_t height, bool has_alpha = true) {
        ...
        if (has_alpha) {
            ...
            file_header.file_size = (int32_t) (file_header.offset_data + data.size());
        } else {
            bmp_info_header.size = sizeof(BMPInfoHeader);
            file_header.offset_data = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader);
            ...
            uint32_t new_stride = make_stride_aligned(4);
            file_header.file_size = (uint32_t) (
...
private:
    ...
    bool check_color_header(BMPColorHeader &bmp_color_header) {
        ...
        if (expected_color_header.red_mask != bmp_color_header.red_mask ||
            ...
            printf("R: Expected %16X, got %16X\n", expected_color_header.red_mask, bmp_color_header.red_mask);
            printf("G: Expected %16X, got %16X\n", expected_color_header.green_mask, bmp_color_header.green_mask);
            printf("B: Expected %16X, got %16X\n", expected_color_header.blue_mask, bmp_color_header.blue_mask);
            printf("A: Expected %16X, got %16X\n", expected_color_header.alpha_mask, bmp_color_header.alpha_mask);
            throw std::runtime_error(
                    "Unexpected color mask format! The program expects the pixel data to be in the BGRA format");
        }
        if (expected_color_header.color_space_type != bmp_color_header.color_space_type) {
            char *got = (char *) &bmp_color_header.color_space_type;
            return got[3] == 'W' && got[2] == 'i' && got[1] == 'n' && got[0] == ' ';
        }
    }
};
